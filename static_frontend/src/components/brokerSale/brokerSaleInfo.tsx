...
function saleStatus(currentBlockNumber: number, saleInfo: SaleInfoType, config: ConfigurationType, constant: BrokerConstantsType) {
    ...
    if (currentBlockNumber < saleInfo.saleStart) {
        ...
        timeRemaining = blocksToTimeFormat(saleInfo.saleStart + config.leadinLength - currentBlockNumber, typeOfChain);
  ...
  export default function BrokerSaleInfo() {
    ...
    useEffect(() => {
        if (saleInfo && configuration && brokerConstants) {
            const { statusMessage, timeRemaining } = saleStatus(currentBlockNumber, saleInfo, configuration, brokerConstants);
