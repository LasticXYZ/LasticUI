...
const Slider: FC<SliderPropeTypes> = ({ currentRelayBlock, saleInfo, config, constants }) => {
    ...
    const utilizationPercentage = ((currentRelayBlock - saleInfo.regionBegin * constants.timeslicePeriod) / (saleDurationOnRelay)) * 100;
    ...
    return (
      <div className='mx-20 my-16 relative'>
        ...
        <div className="w-full bg-pink-4 bg-opacity-20 h-3 rounded-full overflow-hidden">
          <div className="bg-pink-4 bg-opacity-50 h-full" style={{ width: `${safeutilizationPercentage}%` }}></div>
        </div>
