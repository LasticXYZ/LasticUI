...
const Slider: FC<SliderPropeTypes> = ({ currentBlockNumber, saleInfo, config, constants }) => {
  ...
  return (
    <div className="mx-10 my-16 relative">
      <div className="w-full bg-pink-4 dark:bg-pink-9 dark:bg-opacity-20 bg-opacity-20 h-3 rounded-full overflow-hidden">
        <div
          className="bg-pink-4 dark:bg-pink-8 dark:bg-opacity-40 bg-opacity-50 h-full"
          style={{ width: `${safePurchasePercentage}%` }}
        ></div>
      </div>
      {/* Marker for Interlude Period */}
      <div className="absolute top-0 -mt-1" style={{ left: `${0}%` }}>
        <div className="w-5 h-5 bg-red-4 dark:bg-pink-9 rounded-full"></div>
