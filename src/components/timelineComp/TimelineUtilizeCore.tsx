...
const Slider: FC<SliderPropeTypes> = ({ currentRelayBlock, beginRegion, config, constants }) => {
  ...
  return (
    <div className="mx-10 mt-16 mb-10 relative">
      ...
      <div className="w-full bg-pink-4 dark:bg-teal-4 bg-opacity-20 dark:bg-opacity-20 h-3 rounded-full overflow-hidden">
        <div
          className="bg-pink-4 dark:bg-teal-4 bg-opacity-50  dark:bg-opacity-50 h-full"
          style={{ width: `${safeutilizationPercentage}%` }}
        ></div>
      </div>
      {/* Marker for Interlude Period */}
      <div className="absolute top-0 -mt-1" style={{ left: `${0}%` }}>
        <div className="w-5 h-5 bg-red-4 dark:bg-teal-4 rounded-full"></div>
