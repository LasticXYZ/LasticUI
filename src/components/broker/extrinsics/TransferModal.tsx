...
const TransferModal: FC<TransferModalProps> = ({ 
  ...
  const coreNbInt = parseInt(coreNb.replace(/,/g, ''), 10);
  // Convert hex string to bytes
  const maskBytes = hexToU8a(mask);

  console.log('beginInt', beginInt);
    console.log('coreNbInt', coreNbInt);
  ...
  const txButtonProps: TxButtonProps = {
    ...
    attrs: {
      palletRpc: 'broker',
      callable: 'transfer',
      inputParams: [{ begin: beginInt, coreNb: coreNbInt, mask: mask }, newOwner],
      paramFields: [
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={`Transfer Core Nb: ${coreNb} To Another Account`}>
        <div className="flex flex-col p-4">
            <div className="flex flex-col mb-4">
                ...
                <p className="text-lg mb-2">From: {activeAccount ? truncateHash(encodeAddress(activeAccount.address, activeChain?.ss58Prefix || 42), 8) : 'error'}</p>
                <label htmlFor="newOwner" className="text-lg font-semibold mb-2">To:</label>
                <input 
                    id="newOwner"
            ...
            <div className="flex justify-center pt-5">
                <PrimaryButton title="Transfer Core" onClick={transaction} disabled={!allParamsFilled()} />
                <div className="mt-5 text-sm text-gray-16 ">
                    {status}
                </div>            
            </div>
        </div>
