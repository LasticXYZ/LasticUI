...
const CoreUtilisation: React.FC = () => {
  ...
  const dataConfigs = {
    price: {
      ...
      dataPoints: result?.data.event?.map(event => parseFloat(event.regularPrice?.toString() || '0')) || [],
    },
    cores: {
      label: 'Cores Offered',
      dataPoints: result?.data.event?.map(event => parseFloat(event.coresOffered?.toString() || '0')) || [],
    },
  };

  const labels = result?.data.event?.map((event, index) => `Nb. ${index + 1} - Rg. ${event.regionBegin}`) || [];

  // Toggle between data sets
  const toggleActiveDataSet = (newDataSet: string) => {
    setActiveDataSet(newDataSet);
  };

  // Generate buttons or options for switching datasets
  const dataSetOptions = Object.keys(dataConfigs).map(key => (
    <button key={key} onClick={() => toggleActiveDataSet(key)} className={`py-2 px-4 text-left hover:font-semibold border-b border-gray-18 ${activeDataSet === key ? 'text-pink-5 font-semibold' : 'text-gray-16'}`}>
      {dataConfigs[key].label}
    </button>
  ));
