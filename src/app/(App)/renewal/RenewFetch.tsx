...
import { AllowedRenewalAssignmentInfo, AllowedRenewalCoreInfoUnf, AllowedRenewalsType } from '@/types';
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const allowedRenewals: AllowedRenewalsType = entries.map(([key, value]) => {
                    ...
                    const assignmentInfo: AllowedRenewalAssignmentInfo = value.toHuman() as AllowedRenewalAssignmentInfo
...
const RenewalsData = () => {
    ...
    const filteredData = allowedRenewals?.filter(plan => {
        return (!core || parseFormattedNumber(plan.coreInfo[0].core) === core) &&
               (!begin || parseFormattedNumber(plan.coreInfo[0].when) === begin) &&
                (!task || parseFormattedNumber(plan.assignmentInfo.completion?.Complete[0]?.assignment.Task) === task);
    });
