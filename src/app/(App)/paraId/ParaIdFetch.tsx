...
import {
  WorkplanAssignmentInfo,
  WorkplanAssignmentInfoUnf,
  WorkplanCoreInfo,
  WorkplanType,
} from '@/types'
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const workplan: WorkplanType = entries.map(([key, value]) => {
          ...
          const coreInfo: WorkplanCoreInfo =
            coreInfoUnf.length > 0 && coreInfoUnf[0].length > 1
              ? {
          ...
          const assignmentInfo: WorkplanAssignmentInfo[] = (
            value.toHuman() as WorkplanAssignmentInfoUnf[]
          ).map((info) => ({
            ...info,
            assignment:
              typeof info.assignment === 'object'
                ? { Task: parseFormattedNumber(info.assignment.Task) }
                : 'Pool',
          }))
          ...
          <>
            ...
            <div className="flex w-full items-center justify-between space-x-2 mt-4 px-5">
              ...
              <button
                ...
                className={`px-4 py-2   border border-gray-21 text-black dark:text-gray-1 font-semibold rounded-2xl ${filteredData.length < itemsPerPage ? 'bg-gray-4 text-gray-18 cursor-not-allowed' : ' hover:bg-green-6'}`}
              >
                Next
              </button>
            </div>
